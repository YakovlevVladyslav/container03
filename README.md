...
В ходе работы была освоена базовая установка и настройка Docker Desktop, а также проверена его работоспособность. Получен практический опыт создания Docker-образа с помощью Dockerfile и запуска контейнера на его основе. Изучены основные команды Docker (build, run, rm) и структура контейнера. Дополнительно было рассмотрено взаимодействие контейнера с файловой системой (копирование файлов в образ). Также выявлены и решены проблемы, связанные с кэшированием командной оболочки.
